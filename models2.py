...
class RankNet(nn.Module):
    ...
    def forward(self, img):
        ...
        b1 = self.distortion_conv[0](res_out)
        attention_q1 = torch.sigmoid(b1 @ q1)
        attention_b1 = torch.sigmoid(q1 @ b1)

        q2 = self.quality_conv[1](q1 * attention_q1)
        b2 = self.distortion_conv[1](b1 * attention_b1)
        attention_q2 = torch.sigmoid(b2 @ q2)
        attention_b2 = torch.sigmoid(q2 @ b2)
        q3 = self.quality_conv[2](q2 * attention_q2)
        b3 = self.distortion_conv[2](b2 * attention_b2)
        attention_q3 = torch.sigmoid(b3 @ q3)
        attention_b3 = torch.sigmoid(q3 @ b3)
        q4 = self.quality_conv[3](q3 * attention_q3)
        b4 = self.distortion_conv[3](b3 * attention_b3)

        out = torch.cat([q4, b4], dim=1)
        out = F.relu(out)
        out = F.avg_pool2d(out, [7, 7]).squeeze(3).squeeze(2)

        # x1 = self.quality_conv(res_out).squeeze(3).squeeze(2)  # b,128
        # x2 = self.distortion_conv(res_out).squeeze(3).squeeze(2)  # b,128
        # out = x1 * x2
